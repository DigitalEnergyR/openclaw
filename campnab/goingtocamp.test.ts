...
import { type FetchLike, localizedName, GoingToCampClient } from "./goingtocamp.ts";

function jsonResponse(body: unknown, status = 200) {
  return { ok: status >= 200 && status < 300, status, json: async () => body };
}

describe("GoingToCampClient.getMapAvailability", () => {
...
describe("GoingToCampClient retry behavior", () => {
  it("retries on 5xx then succeeds", async () => {
    ...
    const client = new GoingToCampClient({ fetchImpl, sleep: async () => {} });
  ...
  it("gives up after exhausting retries on persistent 5xx", async () => {
    ...
    const client = new GoingToCampClient({ fetchImpl, retries: 2, sleep: async () => {} });
